<div x-data="{ open: false }" class="">
    ...
    <div
        id="modal-EditSubProgram"
        class="hidden fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
        <div class="bg-white p-5 rounded-lg shadow-lg w-96">
            <!-- Header -->
            <div class="flex justify-between items-center border-b pb-2">
                <h2 class="text-lg font-semibold">Edit Sub Program</h2>
                <button
                    onclick="closeModal('modal-EditSubProgram')"
                    class="text-gray-500 hover:text-gray-700 text-xl">&times;</button>
            </div>

            <!-- Form -->
            <form action="{{ route('sub_program.update', 1) }}" method="POST" class="mt-4">
                @csrf
                @method('PUT')
                <input type="hidden" id="edit_sub_program_id" name="sub_program_id">
